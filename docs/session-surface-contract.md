...
A promised scope is opened by a no-selector session whose assessed state
contains exactly one seed `intent.target`. The target is read from assessed
state and resolved through the promised/ticket route; per ADR-0001 D1, scope is
derived at the entry boundary, and the seed-target route obtains its reference
from assessed state rather than a flag.
