...
Scope is methodology-owned metadata, not something runa infers from schemas or artifact contents. Unscoped commands evaluate only unscoped protocols. `runa go` with no selector advances one unscoped session tick through the same session MCP surface. `runa state --work-unit <ID>`, `runa step --work-unit <ID>`, `runa run --work-unit <ID>`, and `runa go --work-unit <ID>` evaluate only `scoped = true` protocols for that delegated work unit.
...
To start scoped work from a tracker ticket, seed a valid unscoped `intent`
artifact whose optional `target` field is a forge ticket reference (a bare
number, `#<N>`, `owner/repo#<N>`, an issue URL, or
`sourcehut:<tracker_id>#<N>`), then run `runa run` or `runa go` with no selector.
The runtime resolves the reference to an identity and serves the methodology's
acquisition surface; the methodology reads the ticket and materializes the
`work-unit`, after which the session is indistinguishable from one opened on a
recorded work-unit. The runtime performs no forge read of its own.
...
| `runa go` | Advance one interactive session tick through the session MCP surface |
